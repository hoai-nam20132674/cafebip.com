...
<div id="page-wrapper">
    ...
    <form action="{{ $pro->id }}" method="post" name="edit_pro_form" enctype="multipart/form-data">
        ...
        <label>{{ trans('admin.image')}}</label></br>
            <img id="ennavatar" src="{{ asset('img/product/'.$pro->image) }}" class="img-thumbnail" style="width: 100%;">
            <hr>
            @foreach($pimage as $im)
            <div class="col-xs-6 form-group edit_form_img" id="{{ $im->id }}" >
                <img src="{{ asset('img/product/Pdetail/'.$im->image)  }}"  class="edit_pimage" idhinh="{{ $im->id  }}" id="{{  $im->id  }}" >
...
@section('script')
  <script src="{{ asset('js/admin/addcontentpro.js') }}"></script>
@endsection()
<style type="text/css">
    .edit_form_img{
        position: relative;
